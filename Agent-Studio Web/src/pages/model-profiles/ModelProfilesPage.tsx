...
export function ModelProfilesPage() {
  ...
  return <div>
    <PageHeader title="Model Profiles" description="DeepSeek API Key 先预留；沙盒阶段使用 Mock LLM。" actions={<Button type="primary" onClick={() => openForm()}>新建模型配置</Button>} />
    <Card className="ios-card"><Table rowKey="id" loading={profiles.isLoading} dataSource={profiles.data?.items ?? []} columns={[
      ...
      { title: '操作', render: (_, r) => <Space><Button onClick={() => openForm(r)}>编辑</Button><Button onClick={() => test.mutate(r.id)}>测试</Button><Button onClick={() => setDefault.mutate(r.id)}>设默认</Button></Space> },
    ]} /></Card>
